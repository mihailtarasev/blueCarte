...
static NSString const * GET_REST = @"restaurants/?";
...
{
    NSURL *url=[NSURL URLWithString:[NSString stringWithFormat:@"%@%@format=json",BASE_URL, GET_REST]];
    [self asynGetReq:@"GET"
                 url:url
              params:nil
            callback:^(id sender){
                ...
                [PECModelDataCards setObjJSON:[PECBuilderModel groupsFromJSON:responseData error:&nError objectModel:@"PECModelDataCards"]];
...
- (void)getUserlDataServerFromEmai:(NSString*)email callback:  (void (^)(id)) callback
{
    ...
    [self asynGetReq:@"GET"
            ...
            callback:^(id sender){
                 ...
                 NSError *nError;
                //[PECModelsData setModelUser:
                
                NSLog(@"responseData %@", responseData);
                
                
                [PECBuilderModel parserJSONUsers:responseData error:&nError];
